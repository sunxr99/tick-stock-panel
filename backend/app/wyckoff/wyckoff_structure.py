...
private deployment.  It intentionally returns observation data only; it does
not alter any formal strategy selection until Layer 4 is migrated and wired.
"""
...
from app.wyckoff._price_math import to_numeric as _to_numeric
...
def _range_zone(df: pd.DataFrame, *, lookback: int, min_bars: int, exclude_last: int) -> pd.DataFrame | None:
...
def _range_width_ok(support: float, resistance: float, cfg: FunnelConfig) -> float | None:
    ...
    max_width = min(max(float(cfg.spring_tr_max_range_pct) * 1.5, 24.0), 55.0)
...
def _range_drift(close: pd.Series, cfg: FunnelConfig) -> tuple[float, float] | None:
    ...
    max_drift = max(float(cfg.spring_tr_max_drift_pct) * 1.5, 18.0)
...
def _range_quality(
    width_pct: float, drift_pct: float, max_drift: float, support_tests: int, resistance_tests: int, atr_pct: float
) -> float:
...
def _range_candidate(
    ...
    return _RangeCandidate(
        ...
        atr_pct=_range_atr_pct(zone, cfg.spring_tr_atr_window),
    )
...
def identify_trading_range(
    ...
    *,
    ...
    """Identify a recent Wyckoff trading range from swing highs and lows."""
...
def _infer_stage(df: pd.DataFrame, tr: TradingRange, trigger_keys: set[str]) -> str:
    last_close = float(_to_numeric(df["close"]).iloc[-1])
    width = tr.resistance - tr.support
    if last_close >= tr.resistance:
        return "Markup"
    if "spring" in trigger_keys or "lps" in trigger_keys:
        return "Accum_C"
...
def _sos_trigger_score(series: _StructureSeries, bar: _LastBar, tr: TradingRange, cfg: FunnelConfig) -> float | None:
    ...
    if vol_ratio is None:
        return None
    structure_breakout = bar.last_close >= tr.resistance * (1.0 - float(cfg.sos_breakout_tolerance))
    enough_push = bar.last_pct >= float(cfg.sos_pct_min)
    enough_volume = vol_ratio >= float(cfg.sos_vol_ratio)
...
def _spring_trigger_score(series: _StructureSeries, bar: _LastBar, tr: TradingRange, cfg: FunnelConfig) -> float | None:
    ...
    enough_volume = vol_ratio is not None and vol_ratio >= float(cfg.spring_vol_ratio)
...
def detect_structure_triggers(
    symbols: list[str], df_map: dict[str, pd.DataFrame], cfg: FunnelConfig, *, lookback: int = 90
) -> StructureTriggerResult:
    """Run dynamic-trading-range Spring, SOS, LPS and EVR detection."""
...
def build_structure_shadow(
    legacy_triggers: dict[str, list[tuple[str, float]]], structure_result: StructureTriggerResult, *, universe_count: int
) -> dict:
    """Compare dynamic structure signals without promoting them to selection."""
    comparisons = {}
